...
const RemoveActivation: React.ComponentType<IRemoveActivation> = ({
  ids = [],
  fetchData = null,
  ...
  async function removeActivations(ids) {
    return Promise.all(ids.map(async (id) => await removeActivation(id)));
  }
  ...
  return (
    <Modal
      aria-label={intl.formatMessage(sharedMessages.activationRemoveTitle) as string}
      titleIconVariant="warning"
      title={
        removeId
          ? intl.formatMessage(sharedMessages.activationRemoveTitle)
          : intl.formatMessage(sharedMessages.activationsRemoveTitle)
      }
      isOpen
      variant="small"
      onClose={goBack}
