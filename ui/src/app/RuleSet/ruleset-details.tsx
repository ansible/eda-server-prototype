          ...
          <Flex direction={{ default: 'column' }} flex={{ default: 'flex_1' }}>
            <FlexItem>
              <Stack>
                ...
                <StackItem>{ruleset?.name}</StackItem>
              </Stack>
            </FlexItem>
            <FlexItem>
              <Stack>
                <StackItem>
                  <Title headingLevel="h3">{intl.formatMessage(sharedMessages.number_of_rules)}</Title>
                </StackItem>
                <StackItem>{ruleset?.rule_count}</StackItem>
              </Stack>
            </FlexItem>
            <FlexItem>
              <Stack>
                ...
                <StackItem>
                  <Text component={TextVariants.h5}>
                    {new Intl.DateTimeFormat('en-US', { dateStyle: 'short', timeStyle: 'long' }).format(
                      new Date(ruleset?.modified_at || 0)
                    )}
                  </Text>
                </StackItem>
              </Stack>
          ...
          <Flex direction={{ default: 'column' }} flex={{ default: 'flex_1' }}>
            <FlexItem>
              <Stack>
                ...
                <StackItem>{ruleset?.description}</StackItem>
              </Stack>
            </FlexItem>
            <FlexItem>
              <Stack>
                ...
                <StackItem>
                  <Link to={'/project/' + ruleset?.project?.id}>
                    {ruleset?.project?.name || `Project ${ruleset?.project?.id}`}
                  </Link>
                </StackItem>
              </Stack>
            </FlexItem>
          ...
          <Flex direction={{ default: 'column' }} flex={{ default: 'flex_1' }}>
            <FlexItem>
              <Stack>
                ...
                <StackItem>
                  <Link to={'/rulebooks/rulebook/' + ruleset?.rulebook?.id}>
                    {ruleset?.rulebook?.name || `Project ${ruleset?.rulebook?.id}`}
                  </Link>
                </StackItem>
              </Stack>
            </FlexItem>
  ...
  return (
    <PageSection page-type={'ruleset-details'} id={'ruleset-details'}>
      ...
      <Stack>
        <StackItem>
          <Card>
            <CardBody>{renderFlexRulesetDetails(ruleset)}</CardBody>
          </Card>
        </StackItem>
      </Stack>
    </PageSection>
