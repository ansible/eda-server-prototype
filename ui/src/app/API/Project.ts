...
export const listProjects = (pagination = defaultSettings) => fetch(projectsEndpoint, {
  headers: {
    'Content-Type': 'application/json',
  },
}).then(response => response.json());
export const fetchProject = (id: string| number | undefined) => fetch(`${projectsEndpoint}/${id}`, {
  headers: {
    'Content-Type': 'application/json',
  },
}).then(response => response.json());
...
export const updateProject = (project: ProjectUpdateType ) => patchData(`${projectsEndpoint}/${project.id}`,
