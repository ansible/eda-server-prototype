...
export const fetchJob = (id: string | number | undefined) =>
  fetch(`${jobEndpoint}/${id}`, {
    headers: {
      'Content-Type': 'application/json',
    },
  }).then((response) => response.json());

export const fetchJobEvents = (id: string | number) =>
  fetch(`${eventsEndpoint}/${id}`, {
    headers: {
      'Content-Type': 'application/json',
    },
  }).then((response) => response.json());
...
export const listJobs = (pagination = defaultSettings) =>
  fetch(jobsEndpoint, {
    headers: {
      'Content-Type': 'application/json',
    },
