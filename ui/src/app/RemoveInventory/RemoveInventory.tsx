...
const RemoveInventory: React.ComponentType<IRemoveInventory> = ({
  ids = [],
  fetchData = null,
  ...
  async function removeInventories(ids) {
    return Promise.all(ids.map(async (id) => await removeInventory(id)));
  }
  ...
  return (
    <Modal
      aria-label={intl.formatMessage(sharedMessages.inventoryRemoveTitle) as string}
      titleIconVariant="warning"
      title={
        removeId
          ? intl.formatMessage(sharedMessages.inventoryRemoveTitle)
          : intl.formatMessage(sharedMessages.inventoriesRemoveTitle)
      }
      isOpen
      variant="small"
      onClose={goBack}
