...
start-events-services() {
  ...
  if [[ $(docker inspect --format '{{json .State.Running}}' eda-postgres) = "true" ]]; then
    ...
    log-debug "docker-compose -p ansible-rulebook -f tools/docker/docker-compose.yml up -d postgres"
    docker-compose -p ansible-rulebook -f tools/docker/docker-compose.yml up -d postgres
...
stop-events-services() {
  ...
  if [[ $(docker inspect --format '{{json .State.Running}}' eda-postgres) = "true" ]]; then
    log-debug "docker-compose -p ansible-rulebook -f tools/docker/docker-compose.yml down"
    docker-compose -p ansible-rulebook -f tools/docker/docker-compose.yml down
  fi
}
...
clean-events-services() {
  ...
  if docker volume inspect -f '{{.Name}}' ansible-rulebook_postgres_data| grep ansible-rulebook_postgres_data > /dev/null 2>&1; then
    log-debug "docker volume rm ansible-rulebook_postgres_data"
    docker volume rm ansible-rulebook_postgres_data > /dev/null 2>&1
  fi
}
